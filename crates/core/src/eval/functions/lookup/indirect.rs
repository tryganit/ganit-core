...
mod tests {
    ...
    fn a1_ref_returns_empty() {
        assert_eq!(
            indirect_fn(&[Value::Text("A1".to_string())]),
            Value::Empty
        );
    }
    ...
    fn b10_ref_returns_empty() {
        assert_eq!(
            indirect_fn(&[Value::Text("B10".to_string())]),
            Value::Empty
        );
    }
    ...
    fn r1c1_ref_returns_empty() {
        assert_eq!(
            indirect_fn(&[Value::Text("R1C1".to_string())]),
            Value::Empty
        );
    }

    #[test]
    fn explicit_a1_true_returns_empty() {
        assert_eq!(
    ...
    fn empty_string_returns_ref_error() {
        assert_eq!(
            indirect_fn(&[Value::Text("".to_string())]),
            Value::Error(ErrorKind::Ref)
        );
    }
    ...
    #[test]
    fn no_args_returns_na_error() {
        assert_eq!(indirect_fn(&[]), Value::Error(ErrorKind::NA));
    }

    #[test]
    fn a1_false_mode_returns_empty() {
        // second arg is the a1 flag; false means R1C1 mode, but "A1" is also a valid A1 ref
        assert_eq!(
