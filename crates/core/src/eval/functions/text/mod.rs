...
pub fn register_text(registry: &mut Registry) {
    ...
    registry.register_eager("ROMAN",       roman::roman_fn,            FunctionMeta { category: "text", signature: "ROMAN(number, [style])",                        description: "Convert integer to Roman numeral string" });
    ...
    registry.register_eager("SPLIT",       split_fn::split_fn,         FunctionMeta { category: "text", signature: "SPLIT(text, delimiter, [split_by_each], [remove_empty])", description: "Split text into array by delimiter" });
