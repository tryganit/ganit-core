...
//! # Fixture directories
//!
//! `tests/fixtures/google_sheets/` — immutable oracle data produced by the
...
//! can merge.  `bugs.tsv` is also immutable oracle data; its rows are known
//! gaps and produce a non-blocking progress report.
//!
//! `tests/fixtures/lab/` — staging area for test cases discovered during
//! development, not yet canonicalized through the oracle pipeline.  Lab tests
...
//! against the oracle value.  Number comparisons allow 1e-4 relative tolerance.
...
///
/// Rows here are confirmed bugs from oracle captures — formulas where Google
/// Sheets returns a specific value but our engine does not yet agree.  The
/// test does NOT panic on failures; a failure means the gap is still open.
/// When the engine is fixed, the row's pass count rises automatically.
/// Do NOT edit this file by hand — it is immutable oracle data.
#[test]
fn bugs_conformance() {
...
/// discovered during development that have not yet been canonicalized through
/// the Google Sheets oracle pipeline.  Failures do NOT block CI; they mean a
/// known case is still open.  See `tests/fixtures/lab/README.md` for the full
/// intent and graduation process.
#[test]
fn lab_conformance() {
    let dir = lab_dir();
